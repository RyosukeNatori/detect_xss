...
import type { typeScope, typeFunctionScope } from './type';
...
export const getAst = (phpfilePath: string): any => {
  let parsedCode = null;
  try {
  ...
  return parsedCode as any;
};

let scope: typeScope | typeFunctionScope | null = null;
export const buildScopeObject = ({
  ast,
  target,
  parent,
}: {
  ast: any;
  target?: typeScope[] | typeFunctionScope[];
  parent?: typeScope | typeFunctionScope;
}): typeScope | void => {
  const templateScope: typeScope = {
    kind: '',
    parent: null,
    childrenScopes: [],
    variables: new Map(),
  ...
  const templateFunction: typeFunctionScope = {
    kind: 'function',
    name: '',
    childrenScopes: [],
    variables: new Map(),
